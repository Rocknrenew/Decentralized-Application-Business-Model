...
1.  The application must be completely open-source. It must operate autonomously, with no entity controlling the majority of its tokens. Its data and records of operation must be cryptographically stored in a public, decentralized block chain.
...
The non-profit can make fully decentralized decisions, allowing a “proof of stake” voting mechanism to determine any decision.
...
There are several categories for which tokens may be issued:
...
2.  Gain Community Engagement: Release the plan to the community and revise based on feedback.
